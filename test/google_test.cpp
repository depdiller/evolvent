...
TEST(EvolventMethods, Parameters) {
    lab2::Evolvent a1;
    const double err = 0.00001, sqrt2 = 1.41421, PI = 3.141459;
    ASSERT_NEAR(sqrt2, a1.ro(1), err);
    lab2::Point p;
    p = a1.polarToDec(PI);
    ASSERT_NEAR(1, p.getX(), err);
    ASSERT_NEAR(-PI, p.getY(), err);
    ...
    ASSERT_STREQ("x =1.00 * (cos(t) + t * sin(t))\ny = 1.00 * (sin(t) - t * cos(t))\n", a1.form());
    ASSERT_STREQ("x = 0\ny = 0\n", a2.form());
}
